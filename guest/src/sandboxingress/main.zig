...
fn scanVirtioPorts() !?std.fs.File {
    ...
    while (try it.next()) |entry| {
        ...
        if (!virtioPortMatches(entry.name, "virtio-ingress")) continue;
...
fn openVirtioPort() !std.fs.File {
    const paths = [_][]const u8{
        "/dev/virtio-ports/virtio-ingress",
    };
    ...
    while (true) {
        for (paths) |p| {
            if (try tryOpenVirtioPath(p)) |file| return file;
        }

        if (try scanVirtioPorts()) |file| return file;

        if (!warned) {
            log.info("waiting for virtio-ingress port", .{});
            warned = true;
        }
